...
async def get_wardrobe_items(
    ...
    # Supports multiple filter types for wardrobe organization
    # Category and search filters are applied at database level
    # Color and season filters are applied post-query for array field matching
    ...
    try:
        ...
        # Search uses PostgreSQL ilike for name and array containment for labels
        if search:
            query = query.or_(f"name.ilike.%{search}%,labels.cs.{{{search}}}")

        result = query.order("created_at", desc=True).execute()

        # Post-query filtering for array fields (colors, seasons)
        # Supabase doesn't support complex array filtering in query
        items = []
...
async def get_wardrobe_item(item_id: str, user_id: str):
    ...
    # Verifies user ownership before returning item
...
async def create_wardrobe_item(
    ...
    # 2. Reference existing image path (used after background removal)
    ...
    try:
        ...
        if not file_path and image:
            file_ext = (
                image.filename.split(".")[-1].lower() if image.filename else "jpg"
            )
        ...
        if not file_path:
            raise HTTPException(
                status_code=400, detail="Either image_path or image file is required"
            )
...
async def update_wardrobe_item(item_id: str, user_id: str, updates: UpdateWardrobeItem):
    ...
    # Image path cannot be updated (would require re-processing)
...
async def delete_wardrobe_item(item_id: str, user_id: str):
    ...
    # Deletes both database record and associated storage file
    ...
    try:
        ...
        supabase.table("wardrobe_items").delete().eq("id", item_id).eq(
            "user_id", user_id
        ).execute()
